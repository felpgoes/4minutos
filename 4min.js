...
(() => {
    const main = function ($) {
        ...
        const replaceImages = (lstImgs, item) => {
        ...
        const replaceVideos = (lstVideos, item) => {
            if ($.inArray($(item).attr('src'), lstVideos) == -1) {
                ...
                if (h > 0 && w > 0) {
                    //Replace
                    $(item).css('width', w + 'px').css('height', h + 'px');
                    $(item).attr('src', lstVideos[Math.floor(Math.random() * lstVideos.length)]);
                    $(item).attr('poster', lstVideos[Math.floor(Math.random() * lstVideos.length)]);
        ...
        const handleImages = (images, time) => {
            setInterval(() => {
        
                $.each($('img'), (i, item) => {
                    //Skip if image is already replaced
                    replaceImages(images, item)
                });
                $.each($('video'), (i, item) => {
                    //Skip if image is already replaced
                    replaceVideos(images, item)
                });
            }, time)
        }

        const fourMinutesImgs = [
            ...
            'https://s02.video.glbimg.com/x720/11192289.jpg'
        ]

        //Run on jQuery ready
        $(function () {
            handleImages(fourMinutesImgs, 3000);
        });
    };
